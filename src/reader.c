...
void _reader_get_from_file(struct banter_state *state, struct banter_data *data) {
    ...
    /* seek to end & record file size in data */
    fseek(state->resource, 0, SEEK_END);
    data->resourceSize = ftell(state->resource);
    /* no need to set back, another seek set will be performed below */
  ...
  data->og_data = (char *)malloc(sizeof(char) * state->stride * state->scale);

  /* Set offset position */
  if(fseek(state->resource, state->offset, SEEK_SET) != 0) {
    /* Unable to seek, just mark we haven't read anything?? */
    printf("CORE ERROR: Unable to seek to offset %d\n", state->offset);

  }

  /* Read data, hold the count as well */
  data->count = fread(data->og_data, sizeof(char), state->stride * state->scale, state->resource);
